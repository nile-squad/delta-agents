...
- **Workflow pause/resume correctness:** phase checkpoints are always-on (the `Phase.checkpoint` flag is declaration-compat only), and a post-step escalation writes a positional checkpoint (`currentPhase` + `currentActionIndex` + `currentActionViaJump`) — workflow resume never re-executes completed phases or already-succeeded actions. The resume checkpoint clears the escalated risk flag (resume is the human resolution, matching the free loop).
- **Bounded invalid-decision feedback:** a model decision naming an unknown action or failing schema validation no longer kills the task — the rejection is fed back via `ReasonerInput.lastError` (user message only, cache-safe) up to `maxInvalidDecisionRetries` consecutive attempts (engine config, default 3; 0 = fail immediately). Any valid decision resets the counter (`TaskStateSnapshot.lastDecisionError`).
- **Trust-based approval waiver:** `Action.requiresApproval` accepts `{ untilTrust }` — approval is required until the task's evidence-derived trust score reaches the threshold, then auto-approved with an auditable ApprovalRequest (`recordAutoApproval`). Rejection always wins (never waived); trust is per-task. Applied in both the reasoner loop and the workflow pre-flight; the gateway treats the object shape as approval-gated (`approvalRequired` helper).
- **Rejection with reason:** `delta.reject(approvalId, reason?)` persists `ApprovalRequest.rejectionReason`. A rejected action is never executed or re-requested (prohibition 11), but the task routes around it: the free loop feeds the reviewer's reason back via the bounded invalid-decision loop (`describeRejection`); a workflow with a rejected pre-flight action fails honestly with the reason (deterministic path cannot route around).
- **Untrusted external content:** tool outputs rendered to the model are wrapped in `<<<external-content>>>` markers with a static system-prefix rule (data, never instructions — cache-safe). Data access stance: actions/tools are the consumer-owned data boundary; `delta.dataSource()` declares sources whose operations are governed as ordinary actions.
...
### Decision: Unsend only while unread; eviction drops oldest read first
Unsend (`engine.unsend`) guards on `readAt` unset — the turn-only window makes this deterministic. `mailbox.inboxCap` eviction (opportunistic on `send`/`inbox`) removes oldest **read**, non-recalled messages; unread are never dropped.

### Files
- `src/engine/roster.ts` (new — `computeRosterEntries`, `RosterEntry` re-export), `src/shared/types.ts` (`RosterEntry`, `Message` fields), `src/engine/types.ts` (`roster`/`inbox`/`outbox`/`unsend`, `mailbox` config), `src/engine/create-delta-engine.ts` (wiring + eviction), `src/engine/scheduler.ts` (roster into ReasonerInput, delivery stamps `readAt`), `src/ports/reasoner-port.ts` + `openai-reasoner.ts` (roster in input + render), `src/ports/storage-port.ts` + `in-memory-store.ts` + `cached-store.ts` + `drizzle/{tasks,messages,converters}.ts` + `db/models/{schema,migrate}.ts` (new store methods + columns), `src/index.ts`/`engine/index.ts` (exports).
