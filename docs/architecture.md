...
**Free reasoner loop.** A task with no assigned workflow uses `runSendLoop`, which drives `runScheduler`. On each step: discover legal actions, rank them by Bellman value, retrieve relevant memory, call `reasoner.reason`, receive a decision, and route it. The reasoner can request an action, signal done, delegate to a teammate, mention a teammate, or send a communication. The reasoner drives the sequence; the engine governs every step. The model call is the least reliable boundary, so each step is retried with jittered backoff (configured by `reasonerRetry`); if it still fails after the retries the task escalates to a human (a `reasoner-failure` escalation, paused and resumable) rather than failing outright (see [diagnostics.md](./diagnostics.md)).
...
Delegation is scoped by team. An agent may only delegate to (or mention) an agent that shares its `team`; an agent with no team treats every other agent as a peer (teams are opt-in). The teammate set is computed by `registry.getTeammates` and enforced in the scheduler, so an out-of-team target is rejected even if a reasoner proposes it. A **mention** is a lighter, distinct decision (`{ kind: "mention", mention: { agentName, message } }`): it records a TaskID-attributable agent-to-agent `Message` to a teammate without spawning a child task or handing off work. The note is delivered exactly once: the next time the recipient agent runs, the scheduler folds any undelivered mentions addressed to it (`store.getMessagesByReceiver`) into that step's reasoning context and marks each consumed (`store.markMessageConsumed`), so a mention reaches its recipient one time across all of its tasks.
