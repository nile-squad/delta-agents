...
`resumeTask`: reads the task and checks that its status is `"paused"` or `"pending"`. Reads the latest checkpoint. Reconstructs the `TaskStateSnapshot` from it (or from the task record if no checkpoint exists), forces `status: "running"`, marks the task running in the store, and re-enters `runSendLoop` with the reconstructed snapshot as `startingSnapshot`.

For workflow tasks, resume re-enters from the beginning of the workflow. Mid-workflow checkpoint resume (resuming into the middle of a phase) is deferred.
