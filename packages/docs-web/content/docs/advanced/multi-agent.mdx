...
An agent can unsend a message, but only while it is still unread. The turn-only model gives a clean window: the sender can retract a message before the recipient's next turn reads it. A recalled message is removed from the inbox and excluded from delivery, but remains visible as recalled in the sender's outbox.

```ts
const recalled = await delta.recall({ messageId: "msg_abc123" });
```
