...
|-------|---------|---------------|
| `approval-requested` | `taskId, action, approvalId, reason, timestamp, agentName` | An action needs human sign-off |
| `approval-resolved` | `taskId, action, approvalId, decision, timestamp, agentName` | Approve or reject resolves it |
| `escalation-raised` | `taskId, trigger, reason, timestamp, agentName` | A reasoner failure or budget violation escalates |
| `task-completed` | `taskId, agentName, goal, timestamp` | A task finishes successfully |
| `task-blocked` | `taskId, agentName, reason, timestamp` | A task is waiting on human oversight |
| `task-failed` | `taskId, agentName, reason, timestamp` | A task terminates with failure |
