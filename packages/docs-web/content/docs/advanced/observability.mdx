...
The engine emits typed events for significant occurrences: task lifecycle changes, human oversight signals, and execution milestones. Subscribe to react in real time instead of polling `inspect()`.

```ts
const unsubscribe = delta.events.on("approval-requested", (data) => {
  console.log(`Task ${data.taskId} needs sign-off for ${data.action}: ${data.reason}`);
...
|-------|---------|---------------|
| `approval-requested` | `taskId, action, approvalId, reason` | An action needs human sign-off |
| `approval-resolved` | `taskId, action, approvalId, decision` | Approve or reject resolves it |
| `escalation-raised` | `taskId, trigger, reason` | A reasoner failure or budget violation escalates |
| `task-completed` | `taskId, agentName, goal` | A task finishes successfully |
| `task-blocked` | `taskId, agentName, reason` | A task is waiting on human oversight |
| `task-failed` | `taskId, agentName, reason` | A task terminates with failure |

Engine-internal events (gated by `diagnostics.engine`): `step-start`, `step-end`, `action-start`, `action-end`, `commit-step-attempt`, `commit-step-done`, `commit-step-auto-commit`.
