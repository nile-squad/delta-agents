...
An agent is a named role with a set of allowed actions, optional workflows, skills, and channels. The agent defines what the model can do. The engine defines what the model will be allowed to do at any given state.
...
- **Skills**: knowledge resources the agent reads at runtime.
...
The system never assumes perfect autonomy. Escalation can occur from risk thresholds, Bayesian surprise, policy violations, budget violations, workflow failures, or explicit configuration. Every escalation is auditable.
