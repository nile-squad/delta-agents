...
Yes. Agents communicate through mailboxes (inbox and outbox) with read receipts and recall support. Communication is turn-only: messages never interrupt an agent mid-task.
